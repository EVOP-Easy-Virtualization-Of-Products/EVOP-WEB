...
export default function BlogPostContent({ id }: { id: string }) {
  ...
  const renderContent = (content: Array<{ type: string; value: string | string[] }>) => {
    return content.map((item, index) => {
      if (item.type === 'text') {
        return (
          <div key={index} className="prose prose-lg max-w-none prose-headings:font-bold prose-h1:text-4xl prose-h2:text-3xl prose-h3:text-2xl prose-p:text-gray-600 prose-a:text-blue-600">
