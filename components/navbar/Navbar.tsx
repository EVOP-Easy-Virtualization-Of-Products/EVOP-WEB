...
export function Navbar() {
  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${isScrolled || mobileMenuOpen ? 'bg-[#0d0d12]/90 backdrop-blur-sm' : 'bg-transparent'}`}>
      <div className="container">
        <div className="flex items-center justify-between h-20">
          <div className="flex-shrink-0">
            <a href="#hero" onClick={(e) => scrollToSection(e, '#hero')} className="flex items-center">
              <Image src="/logo.svg" alt="EVOP" width={120} height={40} className="h-10 w-auto" />
          ...
          <div className="hidden md:flex md:items-center md:space-x-8">
            {navigation.map((item) => (
              <a key={item.name} href={item.href} onClick={(e) => scrollToSection(e, item.href)} className="text-gray-300 hover:text-white transition-colors">
                {item.name}
              </a>
            ))}
            ...
            <div className="relative">
              <button 
                className="flex items-center justify-between space-x-2 text-gray-300 bg-gray-600 px-6 py-2 rounded-full" 
                onClick={() => setDropdownOpen(!dropdownOpen)}
                style={{ width: '148px' }} // Menyesuaikan lebar
              >
                <Image
            ...
            <a
              ...
              className="block w-full bg-[#287eff] text-white px-6 py-2 rounded-full hover:bg-[#287eff]/90 transition-colors"
              style={{ width: '148px' }} // Menyesuaikan lebar
            >
              Contact Us
            </a>
      ...
      {mobileMenuOpen && (
        <div className="md:hidden">
          <div className="space-y-1 px-4 pb-4 pt-2">
            {navigation.map((item) => (
              <a key={item.name} href={item.href} onClick={(e) => scrollToSection(e, item.href)} className="block py-2 text-gray-300 hover:text-white transition-colors">
                {item.name}
              </a>
            ))}
            ...
            <div className="relative">
              <button 
                className="flex items-center justify-between space-x-2 text-gray-300 bg-gray-600 px-6 py-2 rounded-full w-full" 
                onClick={() => setDropdownOpen(!dropdownOpen)}
              >
                <Image
                  src={language === 'en' ? "/bahasa/inggris.jpg" : "/bahasa/indo.jpg"}
              ...
              {dropdownOpen && (
                <div className="absolute right-0 mt-2 w-full bg-gray-600 rounded-lg shadow-lg">
                  <button
                    onClick={() => handleLanguageChange('en')}
                    className="flex items-center space-x-2 p-2 w-full text-left text-gray-300 hover:bg-gray-500"
                  >
